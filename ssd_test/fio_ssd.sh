...
threads=("1" "2" "4"  "6" "8" "10" "12" "14" "16" "24" "48")
threads_s=0
threads_len=9
nr_files=1
...
bs_size=("8B" "16B" "32B" "64B" "256B" "1KB" "4KB" "16KB" "64KB" "256KB")
size_s=0
size_len=10
...
branch="-b 1"
depth="-z 1"
file_name="nova_multi"
run_avg="--allow-run-as-root"
run_avg=""
host="--hostfile hostfile"
host=""
iterates=10
dev_name=/dev/nvme0n1
fallocate="--fallocate=0"
runtime=60
    ...
    do
        ...
        do
            ...
            do
                ...
                do
                echo $file_size
                fio --filename=$dev_name -rw=${type[$t]} -ioengine=${ioengine[$io]} -runtime=$runtime -time_based --direct=1 -bs=${bs_size[$size]} -name=test -iodepth=1 -size=$all_size $fallocate  --nrfiles=${nr_files} --cpus_allowed=${cpus[$f]} --numjobs=${threads[$i]} > ${times}_${type[$t]}_${ioengine[$io]}_${bs_size[$size]}_${threads[$i]}_${name[$f]}.results
                done
            done
        done
    done
